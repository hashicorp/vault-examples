...
# =============================================================================
# Vault Development Server Auto-Configuration Script (with Namespace)
# =============================================================================
# This script automates all settings for Vault development server configuration with namespace support
# 
# Usage:
...
setup_environment() {
    ...
    # Namespace name to use (use root if empty)
    NS_NAME="ns1"

    if [ -n "$NS_NAME" ]; then
        # Check if namespace exists, create if not
        if vault namespace list -format=json 2>/dev/null | grep -q "\"${NS_NAME}/\""; then
            log_warning "Namespace '${NS_NAME}' already exists"
        else
            log_info "Creating namespace '${NS_NAME}'..."
            vault namespace create "${NS_NAME}/"
            log_success "Namespace '${NS_NAME}' created"
        fi

        # Set environment variable to perform subsequent operations in specified namespace
        export VAULT_NAMESPACE="${NS_NAME}"
    else
        # If empty (root), do not set namespace
        unset VAULT_NAMESPACE
        log_info "Using root namespace (no VAULT_NAMESPACE set)"
    fi
...
main() {
    echo "============================================================================="
    echo "🏗️  Vault Development Server Auto-Configuration Script (with Namespace)"
    echo "============================================================================="
    echo ""
